...
class AVLTree {
    ...
    private:
        ...
        bool insert_impl(const int k, ValueType val, int t_id) {
            ...
            TM_SAFE_OPERATION_START {
               ...
               bool found = false;
               ConnPointData<TreeNode> conn_point_snapshot = find_conn_point_custom(k, found);
    ...
    Result<ValueType> lookup(int desired_key) {
        TreeNode* curr;
        for (curr = root; 
                curr && curr->key != desired_key; 
                    curr = desired_key < curr->key ? curr->getL(): curr->getR());

        auto found = curr != nullptr;
        return {found, found ? curr->getValue(): ValueType() };
    }
