...
TEST_CASE("THROUGHPUT TESTS","[tp]") {
    const int OPERATION_MULTIPLIERS[] = {1000000, 10000, 1000,100};
    ...
    for (int i = 0; i < 4; i++) {
        ...
        TestBenchType::test(exp1,THREADS,RANGE_OF_KEYS,1);
        
        // 10 - 10 -80
        TestBenchType::experiment exp2(10,10,80);
        TestBenchType::test(exp2,THREADS,RANGE_OF_KEYS,1);

        // 100% LOOKUPS
        TestBenchType::experiment exp3(0,0,100);
        TestBenchType::test(exp3,THREADS,RANGE_OF_KEYS,1);
        

        // 50-50 UPDATES
        TestBenchType::experiment exp4(50,50,0);
        TestBenchType::test(exp4,THREADS,RANGE_OF_KEYS,1);  
    
        // 25-25 UPDATES, 50 LOOKUPS
        TestBenchType::experiment exp5(25,25,50);
        TestBenchType::test(exp5,THREADS,RANGE_OF_KEYS,1);
